...
{
    public abstract class FirstAbpProjectTestBase : AbpIntegratedTestBase<FirstAbpProjectTestModule>
    {
        private DbConnection _hostDb;
        private Dictionary<int, DbConnection> _tenantDbs; //only used for db per tenant architecture

        protected FirstAbpProjectTestBase()
        {
        ...
        protected void UsingDbContext(Action<FirstAbpProjectDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<FirstAbpProjectDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<FirstAbpProjectDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<FirstAbpProjectDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<FirstAbpProjectDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<FirstAbpProjectDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<FirstAbpProjectDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<FirstAbpProjectDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<FirstAbpProjectDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<FirstAbpProjectDbContext, Task<T>> func)
        {
            T result;
